...
async function handleSavePost() {
  ...
  let imageUrl = existingItem?.imageUrl || existingItem?.photo || '';

  const saveBtn = $('btnSavePost');
  const originalLabel = saveBtn?.textContent || 'Post Listing';
  isSavingPost = true;
  if (saveBtn) {
  ...
  try {
    ...
    const payload = {
      uid: existingItem?.uid || currentUser.uid,
      userEmail: existingItem?.userEmail || currentUser.email || '',
      authorEmail: existingItem?.authorEmail || currentUser.email || '',
      displayName: existingItem?.displayName || currentProfile.displayName || currentUser.email || '',
      authorName: existingItem?.authorName || currentProfile.displayName || currentUser.email || '',
      category: board,
      board,
      status,
      title,
...
function clearPostForm() {
  ...
  const titleEl = document.querySelector('#postOverlay .modal-h strong');
