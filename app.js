...
async function ensureProfile(user) {
  ...
  if (!snap.exists()) {
    ...
    if (currentProfile.isAdmin !== isAdmin(user.email)) {
      await updateDoc(profileRef, { isAdmin: isAdmin(user.email), updatedAt: serverTimestamp() });
...
function updateAuthUI() {
  ...
  $("adminLink").style.display = loggedIn && currentProfile.isAdmin ? "inline-flex" : "none";
  $("btnLogout").style.display = loggedIn ? "inline-flex" : "none";
  $("btnNew").style.display = loggedIn ? "inline-flex" : "none";
...
function renderBoards() {
  ...
  wrap.querySelectorAll(".boardBtn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      $("boardPill").textContent = boardLabels[activeBoard] || "All";
      $("feedTitle").textContent = boardLabels[activeBoard] || "Marketplace";
      renderBoards();
      renderListings();
    });
  });
...
async function openThread(id) {
  ...
  `;
  $("threadReplies").innerHTML = `<div class="note">Loading replies…</div>`;
  $("replyText").value = "";
  show("threadOverlay");
  ...
  repliesUnsub = onSnapshot(qRef, (snap) => {
    ...
    console.error(err);
    $("threadReplies").innerHTML = `<div class="note">Unable to load replies.</div>`;
  });
}
