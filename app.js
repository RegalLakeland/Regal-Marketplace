...
async function handleSavePost() {
  ...
  if (files.length > MAX_POST_IMAGES) {
    alert(`You can upload up to ${MAX_POST_IMAGES} images.`);
    return;
  }

  const totalUploadBytes = files.reduce((sum, file) => sum + (file?.size || 0), 0);
  if (totalUploadBytes > MAX_TOTAL_UPLOAD_BYTES) {
    alert("Selected images exceed the 25MB total upload limit.");
    return;
  }
  ...
  if (existingImages.length + files.length > MAX_POST_IMAGES) {
    alert(`A post can have a maximum of ${MAX_POST_IMAGES} images.`);
    return;
  }
  ...
  try {
    ...
    const allImages = [...existingImages, ...uploadedUrls].slice(0, MAX_POST_IMAGES);
    const payload = {
      uid: currentUser.uid,
...
function clearPostForm() {
  ...
  if ($("postImageNote")) $("postImageNote").textContent = "Upload up to 10 images (25MB total). If you edit a post and select more images, they will be added to the existing gallery.";
}
...
function openEditPost(id) {
  ...
  if ($("postImageNote")) $("postImageNote").textContent = `This post currently has ${existingCount} image${existingCount === 1 ? "" : "s"}. You can add more images up to a total of ${MAX_POST_IMAGES} (25MB total per upload).`;
  show("postOverlay");
}
...
async function openThread(id) {
  ...
  if ($("threadBody")) {
    ...
    const galleryHtml = images.length
      ? `<div class="thread-gallery">${images.map((url) => `<img class="thread-img" src="${esc(url)}" alt="${esc(item.title)}" />`).join("")}</div>`
      : "";
