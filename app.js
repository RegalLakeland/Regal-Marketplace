...
const isAdmin=currentUser.email==="Michael.H@regallakeland.com"
...
await updateDoc(doc(db,"posts",id),{replies:post.replies})
...
render(posts.filter(p=>(p.title+p.desc+p.user).toLowerCase().includes(q)))
