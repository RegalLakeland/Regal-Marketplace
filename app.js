...
function normalizeListing(item) {
  ...
  return {
    ...
    imageUrls: Array.isArray(item.imageUrls) ? item.imageUrls.filter(Boolean) : [item.imageUrl || item.photo || ''].filter(Boolean),
      ...
      <article class="topicRow">
        ...
        <div class="topicSide">
          <div class="topicSideTop">
            ...
            ${getListingImages(item)[0] ? `<img class="topicThumb" src="${esc(getListingImages(item)[0])}" alt="${esc(item.title)}" />` : ''}
          </div>
...
async function handleSavePost() {
  ...
  if (totalBytes > 25 * 1024 * 1024) {
    alert('Total upload size must be 25MB or less.');
    return;
  }
