...
exports.deleteMarketplaceAccount = functions.region("us-central1").https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const requesterEmail = String(decoded.email || "").trim().toLowerCase();
    if (!CORE_ADMINS.has(requesterEmail)) {
      return res.status(403).json({ error: "Only Michael and Janni can permanently delete accounts." });
    }
    ...
    responsesSnap.forEach((docSnap) => batch.delete(docSnap.ref));
    ...
    return res.status(500).json({ error: error.message || "Failed to delete marketplace account." });
  }
});
