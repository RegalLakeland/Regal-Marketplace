...
import { getFirestore, collection, doc, updateDoc, deleteDoc, query, orderBy, onSnapshot } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
function formatDate(v){ if(!v) return ""; const d = v?.toDate ? v.toDate() : new Date(v); return d.toLocaleDateString() + " " + d.toLocaleTimeString([], {hour:"2-digit", minute:"2-digit"}); }
function renderBlocked(msg){ root.innerHTML = `<div class="panel-card"><div class="empty">${esc(msg)}</div></div>`; }
function renderAdmin(users, posts, alerts){
  ...
  document.querySelectorAll(".post-del-btn").forEach(btn => btn.addEventListener("click", async () => {
    if(confirm("Delete this post?")) await deleteDoc(doc(db, "posts", btn.dataset.id));
  }));
}
