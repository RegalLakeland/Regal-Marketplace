...
async function callDeleteMarketplaceAccount(targetUser) {
  ...
  const res = await fetch(deleteAccountFunctionUrl(), {
    ...
    body: JSON.stringify({ uid: targetUser.id, email: normalizeEmail(targetUser.email) })
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(data?.error || `Delete request failed (${res.status})`);
  return data;
}
...
function userPending(user) {
  return !user?.deletedAtMs && (!user.accessApproved || (!user.emailVerified && !user.manualVerified));
}
...
function renderUserRows() {
  ...
  document.querySelectorAll('[data-role]').forEach((btn) => btn.onclick = async () => {
    ...
    if (role === 'permanentDeleteUser') {
      if (!isCoreAdminViewer()) {
        alert('Permanent delete is restricted to Michael and Janni.');
        return;
      }
      if (userFilterValue !== 'DELETED' || !user.deletedAtMs) {
