...
function startListings(){
  ...
  onSnapshot(qRef, (snap) => {
    ...
    document.querySelectorAll('[data-approve]').forEach(btn => btn.onclick = async () => {
      await updateDoc(doc(db, 'listings', btn.dataset.approve), { status:'ACTIVE', reactivationRequested:false, reactivationRequestedAt:null, updatedAt: Date.now() });
    });
    document.querySelectorAll('[data-deny]').forEach(btn => btn.onclick = async () => {
      await updateDoc(doc(db, 'listings', btn.dataset.deny), { reactivationRequested:false, reactivationRequestedAt:null, reactivationDeniedAt: Date.now() });
    });
  ...
  <div class="overlay" id="adminEditOverlay" style="display:none;">
    <div class="modal wide modal-scroll">
      ...
      <div class="modal-b">
        ...
        <div class="grid2">
          <div class="field"><label>Price</label><input id="adminEditPrice" inputmode="decimal" /></div>
          <div class="field"><label>Location</label><input id="adminEditLocation" /></div>
        </div>
...
function closeAdminEdit(){ if ($('adminEditOverlay')) $('adminEditOverlay').style.display = 'none'; }
...
async function saveAdminEdit(){
  ...
  await updateDoc(doc(db, 'listings', id), {
    board, category: board, status, title,
