...
function startListings(){
  ...
  onSnapshot(qRef, (snap) => {
    ...
    $('listingRows').innerHTML = rows.map(item => {
      ...
      return `
        <tr>
          <td><strong>${esc(item.title || 'Untitled')}</strong><div class="meta">${esc(fmtDate(item.createdAtMs))}</div></td>
          <td>${esc(boardLabels[board] || board)}</td>
          <td>${esc(item.status || 'ACTIVE')}</td>
          <td>${esc(poster)}</td>
          <td>
            <div class="rowBtns">
              ${item.status !== 'SOLD' ? `<button class="btn" data-sold="${esc(item.id)}" type="button">Mark Sold</button>` : `<button class="btn ghost" data-active="${esc(item.id)}" type="button">Mark Active</button>`}
    ...
    document.querySelectorAll('[data-active]').forEach(btn => btn.onclick = async () => {
      await updateDoc(doc(db, 'listings', btn.dataset.active), { status:'ACTIVE' });
    });
